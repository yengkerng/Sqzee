$( document ).ready(function() {
   ...
   function displayComment(comment, parent) {
      ...
      $(container).append($(postText), $(userInfo));

      var lengthComments = getPostComments(id).length;
      console.log(lengthComments);

      for (var i = 0; i < lengthComments; i++) {
        var commentDisplay = document.createElement("div");
        $(commentDisplay).addClass("commentContainer").attr("id", getPostComments(id)[i].text);
        $(commentDisplay).text(getPostComments(id)[i].text);

        var separator = document.createElement("span");
        $(separator).addClass("separator").text(" - ");

        var authorDisplay = document.createElement("span");
        $(authorDisplay).addClass("commentContainer").attr("id", getPostComments(id)[i].author);
        $(authorDisplay).text(getPostComments(id)[i].author);

        var timeDisplay = document.createElement("span");
        $(timeDisplay).addClass("commentContainer").attr("id", getPostComments(id)[i].time);
        $(timeDisplay).text(getPostComments(id)[i].time);


        $(authorDisplay).append($(separator));
        $(authorDisplay).append($(timeDisplay));

        $(container).append($(commentDisplay));

        $(container).append($(authorDisplay));
      }
   ...
   function commentStructure(text, author, time) {
    this.text = text;
    this.author = author;
    this.time = time;
  }
   ...
   function getPostComments(postID, type){
      ...
      var action = $.ajax({
         ...
         success  : function(data){
           for (var i = 0 ; i < data.comments.length; i++) {
             ...
             if (data.comments[i].comment != null) {
               comments = data.comments[i].comment.substring(2);
             ...
             var myDate = data.comments[i].timeAgo.split("/");
             var time = myDate[2].split(" ");
             ...
             var timeHour = time[1];
             var amOrPM = time[2];
             console.log(data.comments[i].time);
             timeAgo = data.comments[i].time;
             //author = data.comments[i].timeAgo;
             //console.log(new Date(timeAgo).getTime());
             //timeAgo = data.comments[i].timeAgo;
             //author.push(data.comments[i].author);
             //timeAgo.push(data.comments[i].timeAgo);

           }
           result.push(new commentStructure(comments, author, timeAgo));
      ...
      result.sort(function(a, b) {
          a = new Date(a.timeAgo);
          b = new Date(b.timeAgo);
   ...
   */
   ...
   .mouseout(function() {
      ...
      $(this).css("background-color", "#FE731B");
   });
...
/*
   ...
   function createSingleReview(review) {
      ...
      if (review.rating != "PR") {
         ...
         $(user).addClass("header").attr("id", "reviewTime").text(datePosted.toLocaleDateString());
      }
      else {
